...
4. A `pretty` range, for numeric variables. In this case, `ggpredict()` tries to calculate a pretty numeric range based on how large the range of the related variable is. Example: `terms = "age [pretty]"`. This is what the `pretty`-argument automatically does when a term has more than 25 unique values, so by using `terms = "... [pretty]"` gives you the option to selectively prettify terms.
