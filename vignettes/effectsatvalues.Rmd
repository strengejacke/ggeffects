...
5. Finally, numeric vectors for which no specific values are given, a "pretty range" is calculates (see `pretty_range()`), to avoid memory allocation problems for vectors with many unique values. If a numeric vector is specified as second or third term (i.e. if this vector represents a grouping structure), representative values (see `rprs_values()`) are chosen.
