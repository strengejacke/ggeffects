...
### Conditional predictions: Adjusted predictions for the count model
...
In the following example, we fit a linear mixed model and first simply plot the adjusted predictions on population-level.
...
To compute adjusted predictions for each grouping level (unit-level), add the related random effects term to the `terms`-argument. In this case, predictions are calculated for each level of the random effects.
