...
This vignette shows how to calculate adjusted predictions and marginal effects for mixed models. Like for non-mixed models, where we can calculate _conditional effects_ (`predict_response()` or `predict_response(marginalize = "mean_mode")`) and _(average) marginal effects_ (`predict_response(marginalize = "empirical")`), we can do the same for mixed models.

However, for mixed models, since random effects are involved, we have to distinguish between _population-level_ and _unit-level_ predictions, and between _conditional_ and _marginal_ effects. Additionally, _random effects uncertainty_ can be taken into account, which would lead to _prediction intervals_ instead of _confidence intervals_.
...
- conditional effect: effect of `gdp` in an _average_ or _typical_ country

- marginal effect: average effect of `gdp` _across all_ countries

In a balanced data set, where all groups have the same number of observations, the conditional and marginal effect are similar (slightly different depending on the non-focal predictors). However, in unbalanced data, the conditional and marginal effect can largely differ.
...
predict_response(m, "Days [1,5,9]", marginalize = "empirical")
...
For conditional effects (i.e. when `marginalize` is not set to `"empirical"`), the uncertainty of the random effects can be taken into account. This leads to _prediction intervals_ instead of _confidence intervals_. To do so, we need to set `type = "random"`.

When `type = "random"`, the random-effect _variances_ are taken into account, meaning that the intervals are actually _prediction intervals_ and become larger. More technically speaking, `type = "random"` accounts for the uncertainty of the fixed effects _conditional on the estimates_ of the random-effect variances and conditional modes (BLUPs). Depending on whether `REML = TRUE` or `REML = FALSE` was used for model fitting, the predicted values may differ.
...
It is also possible to obtain predicted values by simulating from the model, where predictions are based on `simulate()`. The predicted values come closer to _marginal effects_, but the intervals come closer to _prediction intervals_.
...
For zero-inflated mixed effects models, typically fitted with the **glmmTMB** or **GLMMadaptive** packages, `predict_response()` (except when `marginalize = "empirical"`) can return predicted values of the response, conditioned on following prediction-types:
...
`predict_response(marginalize = "empirical")` accepts values for `type` based on the model's `predict()` method. For models of class `glmmTMB`, these are `"response"`, `"link"`, `"conditional"`, `"zprob"`, `"zlink"`, or `"disp"`. By default, `type = "response"`, and the returned average marginal effects are closest to `predict_response(type = "simulate")`
...
Average marginal effects for zero-inflated mixed models can be calculated with `marginalize = "empirical"`. The returned values are closest to `predict_response(type = "simulate")`. The next example shows the average marginal effect of `spp` on the response across all `site`s, taking the zero-inflation component into account.

```{r}
predict_response(m, "spp", marginalize = "empirical")
```

## Unit-level predictions (predictions for each level of random effects)

Adjusted predictions can also be calculated for each group level (unit-level) in mixed models. Simply add the name of the related random effects term to the `terms`-argument, and set `type = "random"`. For `predict_response(marginalize = "empirical")`, you don't need to set `type = "random"`.
...
Since average marginal effects already considers random effects by averaging over the groups, the `type`-argument is not needed when `marginalize = "empirical"` is set.

```{r}
me <- predict_response(m, terms = c("c12hour", "e15relat"), marginalize = "empirical")
plot(me, show_ci = FALSE)
```

> When predictions for unit-levels (random effects) are of interest, it can be useful to calculate confidence instead of prediction intervals (i.e. to ignore the uncertainty in the variance parameters and return regular confidence intervals), in particular, when `hypothesis_test()` is used for contrasts or comparisons of unit-levels (aka group levels). In that case, either set `interval = "confidence"` (see examples below), or calcualte average marginal effects using `predict_response(marginalize = "empirical")`.
...
If unit-levels are of interest, setting `type = "random"` is obligatory, unless you use `predict_response(marginalize = "empirical")`. However, sometimes it can be useful to have _confidence_ instead of prediction intervals, e.g. for [pairwise comparisons](introduction_comparisons_1) of random effects. Confidence instead of prediction intervals can be calculated by explicitly setting `interval = "confidence"`, in which case the random effects variances are ignored. `predict_response(marginalize = "empirical")` always returns confidence intervals.
...
# same as
# predict_response(m, terms = c("Days", "Subject [sample=7]"), marginalize = "empirical")
plot(me)
```
