...
For zero-inflated mixed effects models, typically fitted with the **glmmTMB** or **GLMMadaptive** packages, `predict_response()` can return predicted values of the response, conditioned on following prediction-types:
  ...
  - Marginal effects / predictions:
    ...
    * marginal predictions using `margin = "empirical"` are also averaged across all random effects groups and non-focal terms. The major difference to `type = "simulate"` is that `margin = "empirical"` also return _counterfactual_ predictions.
