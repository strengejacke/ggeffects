...
- marginal effect: average effect of `gdp` _across all_ countries (using `predict_response(margin = "ame")`)
...
predict_response(m, "Days [1,5,9]", margin = "ame")
...
For conditional effects (i.e. when `margin` is not set to `"ame"`), the uncertainty of the random effects can be taken into account. This leads to _prediction intervals_ instead of _confidence intervals_. To do so, we need to set `type = "random"`.
...
## Marginal effects and adjusted predictions for zero-inflated mixed models

For zero-inflated mixed effects models, typically fitted with the **glmmTMB** or **GLMMadaptive** packages, `predict_response()` (except when `margin = "ame"`) can return predicted values of the response, conditioned on following prediction-types:
...
Average marginal effects for zero-inflated mixed models can be calculated with `margin = "ame"`. The returned values are most comparable to `predict_response(type = "simulate")`. The next example shows the average marginal effect of `spp` on the response across all `site`s, taking the zero-inflation component into account.

```{r}
predict_response(m, "spp", margin = "ame")
```

## Unit-level predictions (predictions for each level of random effects)

Adjusted predictions can also be calculated for each group level (unit-level) in mixed models. Simply add the name of the related random effects term to the `terms`-argument, and set `type = "random"`. For `predict_response(margin = "ame")`, you don't need to set `type = "random"`.
...
Since average marginal effects already consider random effects by averaging over the groups, the `type`-argument is not needed when `margin = "ame"` is set.

```{r}
me <- predict_response(m, terms = c("c12hour", "e15relat"), margin = "ame")
plot(me, show_ci = FALSE)
```
...
If unit-levels are of interest, setting `type = "random"` is obligatory, unless you use `predict_response(margin = "ame")`. However, sometimes it can be useful to have _confidence_ instead of prediction intervals, e.g. for [pairwise comparisons](introduction_comparisons_1) of random effects. Confidence instead of prediction intervals can be calculated by explicitly setting `interval = "confidence"`, in which case the random effects variances are ignored. `predict_response(margin = "ame")` always returns confidence intervals.
...
# predict_response(m, terms = c("Days", "Subject [sample=7]"), margin = "ame")
plot(me)
```
