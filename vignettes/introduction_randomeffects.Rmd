...
This vignette shows how to calculate adjusted predictions and marginal effects for mixed models. Like for non-mixed models, where we can calculate _conditional effects_ and _(average) marginal effects_, we can do the same for mixed models.

However, for mixed models, since random effects are involved, we also have to distinguish between _population-level_ and _unit-level_ predictions. Additionally, _random effects uncertainty_ can be taken into account, which would lead to _prediction intervals_ instead of _confidence intervals_.
...
- conditional effect: effect of `gdp` in an _average_ or _typical_ country (using `predict_response()` or `predict_response(marginalize = "mean_mode")`)

- marginal effect: average effect of `gdp` _across all_ countries (using `predict_response(marginalize = "empirical")`)
...
Average marginal effects for zero-inflated mixed models can be calculated with `marginalize = "empirical"`. The returned values are most comparable to `predict_response(type = "simulate")`. The next example shows the average marginal effect of `spp` on the response across all `site`s, taking the zero-inflation component into account.
