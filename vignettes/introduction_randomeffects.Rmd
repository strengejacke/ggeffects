...
For non-Gaussian models, predicted values are back-transformed to the resppnse scale. However, back-transforming the population mean ignores the effect of the variation around the population mean (random effects variances), so the result on the original data scale is biased due to _Jensen's inequality_. In this case, it can be appropriate to apply a bias-correction. This is done by setting `bias_correction = TRUE`. In this case, [`insight::get_variance_residual()`](https://easystats.github.io/insight/reference/get_variance_residual.html) is used to extract the residual variance, which is used to calculate the amount of correction. Optionally, you can provide your own estimates of uncertainty using the `sigma` argument.
