...
# and type = "random" in predictions, ony for intervals
...
If random effect levels are of interest, setting `type = "random"` is obligatory. However, sometimes it can be useful to have usual confidence intervals instead of prediction intervals, e.g. for [pairwise comparisons](introduction_comparisons) of random effects. Confidence instead of prediction intervals can be calculated by explicitly setting `interval = "confidence"`, in which case the random effects variances are ignored.
