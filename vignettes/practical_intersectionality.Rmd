...
callout_tip(
  ...
  tags$ul(
    tags$li("The MAIHDA framework is a new gold standard for quantitative intersectional analysis of inequity. This approach involves multilevel modelling, where the combination of inequality dimensions (characteristic of social strata) is included as random effect (level-2 unit).") # nolint
  ),
  tags$p("The analysis includes following steps:"),
  tags$ol(
    tags$li("Fitting a null-model, partially and fully adjusted models. Partially adjusted models each include on of the different inequality dimenstions as fixed effect (level-1)."), # nolint
...
Finally, we may want to have a clearer picture of how the different strata vary, which combination of characteristics defines the highest or maybe lowest risk group. To do so, we calculate average marginal effects of the random effects by setting `margin = "empirical"`.
...
**Note:** to test _random effects_ for significant differences, we need to change the underlying backend to calculate pairwise comparisons. We do this by adding `engine = "ggeffects"` to the `test_predictions()` function.

```{r}
# just show first 10 rows of output...
test_predictions(predictions, engine = "ggeffects")[1:10, ]
```
...
test_predictions(predictions, by = "age", engine = "ggeffects")
```
...
test_predictions(predictions, by = c("gender", "age"), engine = "ggeffects")
```
