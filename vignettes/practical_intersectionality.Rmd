...
To include the intersectional strata variables `gender`, `employed` and `age` in our mixed model, we will define them as interaction random effects (excluding main effects of interactions): `(1 | gender:employed:age)` (see also below). The idea is to have truly unique combinations in our model, similar as if you we create a factor variable with all combinations manually: 
...
```{r}
hypothesis_test(predictions)[36, ]
```
...
```{r}
hypothesis_test(predictions, test = "b4 = b10")
```
