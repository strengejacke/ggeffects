...
Finally, we may want to have a clearer picture of how the different strata vary, which combination of characteristics defines the highest or maybe lowest risk group. To do so, we calculate predictions of the random effects (*unit-level* predictions) by either setting `margin = "empirical"` or setting `type = "random"` (for more defaults how *ggeffects* handles mixed models, see [this vignette](introduction_randomeffects.html).).
