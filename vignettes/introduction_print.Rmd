...
The above shown tables have no theme applied (i.e. `theme = NULL`), however, there are some pre-defined table themes to change the table appearance, too. These can be applied using the `theme` argument, which must be one of `"default"`, `"grid"`,`"striped"`, `"bootstrap"`, or `"darklines"`.
