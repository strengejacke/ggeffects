...
::: custom_note
One crucial distinction is between *conditional* and *marginal* predictions (or effects). Conditional predictions are specific to a particular level of the random effect (e.g., the predicted outcome for a specific individual in a study). Marginal predictions, on the other hand, average over the random effects, providing an overall estimate of the effect in the population. The marginal effect is often the quantity of interest when we want to generalize to the population.

For further reading, we recommend [this new vignette on mixed models](https://easystats.github.io/modelbased/articles/mixed_models.html), and strongly advise using the **modelbased** package for more flexibility.
:::
