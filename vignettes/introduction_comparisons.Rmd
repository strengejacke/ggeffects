...
A reason to compute adjusted predictions (or estimated marginal means) is to help understanding the relationship between predictors and outcom of a regression model. In particular in the case of more complex models, for example, complex interaction terms, it is often easier to understand the associations when looking at adjusted predictions instead of the raw table of regression coefficients.
...
In this simple example, the contrasts of both `episode = 2` and `episode = 3` to `episode = 1` equal the coefficients of the regression table above (same applies to the p-values), where the coefficients refer to the difference between the related parameter of `episode` and its reference level, `episode = 1`.
...
In the above output, each row is considered as one coefficient of interest. Our groups we want to include in our comparison are rows two and five, so our "quantities of interest" are `b2` and `b5`. Our null hypothesis we want to test is whether both predictions are equal, i.e. `test = "b2 = b5"`. We can now calculate the desired comparison directly:
...
For instance, we could now ask whether the predicted outcome for `episode = 1` in the `treatment` groups is significantly different from the predicted outcome for `episode = 3` in the `control` group.

```{r echo=FALSE}
p <- plot(mydf)
line_data <- as.data.frame(mydf)[c(2, 5), 1:2]
...
ht3 <- hypothesis_test(model2, c("episode", "group"))
```
...
## Does difference between two levels of episoe in the control group differ from difference of same two levels in the treatment group?

The `test` argument also allows us to compare differences-in-differences. For example, is the difference between two episode levels in one group significantly different from the difference of the same two episode levels in another group?

```{r echo=FALSE}
mydf <- ggpredict(model2, c("group", "episode"))
p <- plot(mydf)
line_data <- as.data.frame(mydf)[, 1:2,]
