...
We could now ask whether the predicted outcome for `episode = 1` in the `treatment` groups is significantly different from the predicted outcome for `episode = 3` in the `control` group.
...
The contrast we are interested in is between `episode = 1` in the `treatment` group and `episode = 3` in the `control` group.
