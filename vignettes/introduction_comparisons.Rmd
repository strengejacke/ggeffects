...
Contrasts or comparisons are by default on the *response* scale.
...
However, you can use the `scale` argument in `hypothesis_test()` to return contrasts or comparisons on a differen scale. For example, to transform contrasts to _odds ratios_, we can use `scale = "exp"`.
