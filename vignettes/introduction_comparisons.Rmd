...
Contrasts or comparisons are by default on the *link* scale.
...
However, `hypothesis_test()` passes dot-arguments (`...`) down to the functions from *marginaleffects*. Thus, you can use any arguments from `marginaleffects::predictions()` or `marginaleffects::slopes()` in `hypothesis_test()`, too. To transform contrasts or comparisons back as _odds ratios_, we can use the `transform_post` argument.

```{r message=TRUE}
hypothesis_test(p, transform_post = "exp")
```

Contrasts or comparisons can also be represented on the response-scale, in this case as _probabilities_. To do so, use `type = "response"`.

```{r message=TRUE}
hypothesis_test(p, type = "response")
```

## Contrasts and comparisons for numerical focal terms

For numeric focal variables, where the slopes (linear trends) is estimates, these are presented on the response scale (as _probabilities_) by default. The `transform_post` argument is not allowed to be used in such cases.

```{r message=TRUE}
hypothesis_test(m, "x2")
```

However, `type = "link"` can be used to return untransformed contrasts or comparisons on the link-scale.

```{r message=TRUE}
hypothesis_test(m, "x2", type = "link")
```
