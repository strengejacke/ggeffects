...
Instead of character strings, the `vcov.fun` argument also accepts a function that returns a variance-covariance matrix. Further arguments that need to be passed to that functions should be provided as list to the `vcov.args` argument. Thus, we can rewrite the above code-chunk in the following way:
