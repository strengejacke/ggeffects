...
`predict_response()` computes marginal means or predicted values for all possible levels or values from specified model's predictors (_focal terms_). These effects are "marginalized" (or "averaged") over the values or levels of remaining predictors (the **non-focal** terms).

::: custom_note
Broadly speaking, there are two conceptual categories: *conditional* predictions and *marginal* predictions. The first relies on artificially constructed reference grids, providing predictions conditional on specific, theoretical individual levels. In contrast, the latter calculate predictions for the actual empirical observations before averaging, thus reflecting marginal predictions at the sample or population level.

There is an [updated vignette on this topic](https://easystats.github.io/modelbased/articles/technical_marginalization.html). We recommend using the **modelbased** package - a modern rewrite of **ggeffects** that is flexible, easy to use, feature-rich, and provides a clearer and more accurate take on this distinction.
:::

The `margin` argument specifies the method of marginalization. The following methods are available:
