...
Consequently, as `predict_response()` is just a wrapper around the above-mentioned functions, the same applies to `predict_response()` when `margin` is set to `"mean_reference"` (which calls `ggpredict()`) or `"marginalmeans"` (which calls `ggemmeans()`).
