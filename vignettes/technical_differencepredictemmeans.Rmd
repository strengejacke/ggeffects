...
The most apparent difference from `ggaverage()` compared to the other methods, which should be aspect to considered, is whether you have _multi-level models_ (aka _mixed models_) or not. If you have multi-level models, `ggaverage()` will return predictions averaged over the random effects, `ggpredict()`, `ggemmeans()` or `ggeffect()` will not.
...
m <- lmer(neg_c_7 ~ c172code + (1 | e15relat), data = efc)

# predicted values for negative impact, on the population-level
ggpredict(m, "c172code")

# marginal means for negative impact, on the population-level - identical
# to ggpredict() in this case, because we have no other predictors
ggemmeans(m, "c172code")

# replicate results from ggemmeans() with emmeans::emmeans()
emmeans::emmeans(m, "c172code", df = 829)

# predicted values for negative impact, averaged over the sample (i.e.
# also averaged over random effect groups)
ggaverage(m, "c172code")
```
