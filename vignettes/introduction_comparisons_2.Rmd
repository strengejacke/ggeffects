...
callout_tip(
  ...
  tags$ul(
    ...
    tags$li("It is even more complicated to deal with interactions of (at least) two continuous predictors. In this case, the `johnson_neyman()` function can be used, which is a special case of pairwise comparisons for interactions with continuous predictors.") # nolint
  )
)
```
