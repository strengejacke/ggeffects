...
vignette: >
  %\VignetteIndexEntry{Significance Testing of Differences Between Predictions 2: Slopes, Contrasts and Pairwise Comparisons}
...
The results of the spotlight analysis suggest that values below `0.79` and above `2.67` are significantly different from zero, while values in between are not. We can plot predictions at these values to see the differences. The red and the green line represent values of `Illiteracy` at which we find clear positive resp. negative associations between `Murder` and `Income`, while we find no clear association for the blue line.

```{r}
pr <- ggpredict(m_mod, c("Murder", "Illiteracy [0.7, 1.5, 2.8]"))
plot(pr, grid = TRUE)
```
...
[Go to next vignette](https://strengejacke.github.io/ggeffects/articles/introduction_comparisons_2.html)

# Significance Testing of Differences Between Predictions III: Contrasts and Comparisons for Generalized Linear Models

[Go to next vignette](https://strengejacke.github.io/ggeffects/articles/introduction_comparisons_3.html)
