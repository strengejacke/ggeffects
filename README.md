...
statistical models, i.e. **predictions generated by a model when the
non-focalvariables are hold constant (or averaged over) and the focal
variable(s) are varied**. Furthermore, it is possible to compute
contrasts or pairwise comparisons, to test predictions and differences
in predictions for statistical significance. Finally, you can easily
produce nice figures to visualize the results.
