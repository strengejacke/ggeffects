...
.emmeans_prediction_data <- function(
  model,
  data_grid,
  cleaned_terms,
  ci_level,
...
.ggemmeans_MixMod <- function(
  ...
  insight::check_if_installed(
    "emmeans",
    "to compute estimated marginal means for MixMod-models"
  )
  ...
  x1 <- as.data.frame(suppressWarnings(do.call(
    emmeans::emmeans,
    c(arg_list, dot_args)
  )))
  ...
  x2 <- as.data.frame(suppressWarnings(do.call(
    emmeans::emmeans,
    c(arg_list, dot_args)
  )))
...
.ggemmeans_glmmTMB <- function(
  ...
  insight::check_if_installed(
    "emmeans",
    "to compute estimated marginal means for glmmTMB-models"
  )
  # all model variables
...
.ggemmeans_predict_ordinal <- function(
  model,
  data_grid,
  cleaned_terms,
  ci_level,
...
.ggemmeans_predict_MCMCglmm <- function(
  model,
  data_grid,
  cleaned_terms,
  ci_level,
...
.ggemmeans_predict_generic <- function(
  model,
  data_grid,
  cleaned_terms,
  ci_level,
  ...
  if (is.null(tmp)) {
    emmeans_args$data <- insight::get_data(
      model,
      source = "frame",
      verbose = FALSE
    )
...
.ggemmeans_predict_nlme <- function(
  model,
  data_grid,
  cleaned_terms,
  ci_level,
