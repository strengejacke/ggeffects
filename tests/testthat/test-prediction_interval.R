...
test_that("ggpredict prediction interval, glm", {
  ...
  set.seed(123)
  out <- ggpredict(m_pr0, "c172code", type = "simulate", interval = "prediction")
  expect_equal(out$predicted, c(63.01721, 64.71984, 66.49318), tolerance = 1e-3)
  expect_equal(out$conf.high, c(120.48963, 122.27772, 123.5004), tolerance = 1e-3)
  ...
  set.seed(123)
  out <- ggpredict(m_pr1, "mined", type = "simulate", interval = "prediction")
  expect_equal(out$predicted, c(0.29534, 2.26532), tolerance = 1e-3)
  expect_equal(out$conf.high, c(1.90308, 5.60201), tolerance = 1e-3)
  ...
  out <- ggpredict(m_pr1, "mined", type = "simulate", interval = "confidence")
  expect_equal(out$conf.high, c(0.34698, 2.41337), tolerance = 1e-3)
